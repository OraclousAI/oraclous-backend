...
class TestKGSShimChatProxyPath:
    """KGS HTTP shims correctly proxy chat-mode queries to KRS (AC1 proxy path).

    In R3, the chat service may still call the KGS retriever shim while
    migrating. These tests verify the shim routes a chat-style query to the
    correct KRS endpoint rather than failing silently or calling the wrong route.
    """

    async def test_kgs_retriever_shim_makes_http_call_to_krs_retrieve_path(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """KGS RetrieverService.retrieve calls KRS via HTTP for a chat query."""
        import json
        ...
        class _CaptureTransport(httpx.AsyncBaseTransport):
            ...
            async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
                ...
                payload = [{"id": "krs-node-1", "type": "node", "properties": {"text": "result"}}]
                body = json.dumps({"results": payload})
        ...
        monkeypatch.setenv("KRS_BASE_URL", "http://krs-chat-test:8006")
        monkeypatch.setattr(shim, "_transport", cap, raising=False)

        svc = shim.RetrieverService()
        result = await svc.retrieve(query=_CHAT_QUERY)

        assert cap.requests, (
            "KGS RetrieverService.retrieve must call KRS for a chat-mode query; "
            "no HTTP request was captured"
        )
        assert "krs-chat-test:8006" in str(cap.requests[0].url), (
            "Chat-path HTTP call must target KRS_BASE_URL"
        )
        assert result is not None, "Shim must return KRS response to the chat caller"

    async def test_kgs_shim_proxies_chat_query_in_request_body(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """The originating chat query string must appear in the KRS request body."""
        import httpx
        ...
        from oraclous_knowledge_graph_service import (  # ORA-48
            retriever_service as shim,
        )

        monkeypatch.setenv("KRS_BASE_URL", "http://krs-chat-test:8006")
        monkeypatch.setattr(shim, "_transport", _BodyCapture(), raising=False)

        svc = shim.RetrieverService()
        chat_query = "Explain the company's risk management framework"
        await svc.retrieve(query=chat_query)
