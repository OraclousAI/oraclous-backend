...
(503 when the critical store didn't bind). The critical store is Neo4j; a configured-but-failed
bind is degraded, an unset URI is intentional (retrieval 503s but the service is live). No DB access
here — the route reads the bind outcome the lifespan recorded on ``app.state``.
"""
...
_SERVICE = "knowledge-retriever-service"
