"""Short-lived, in-process cache for resolved model credentials (#643 / #949 C2).

Ported from `knowledge-graph-service`'s module of the same name (#724). Resolution costs two
broker round-trips on a fresh connection, and a search request is the hottest path in this
service — without this cache, every single meaning-based search would pay a broker round trip
before it could even embed the query. A load-bearing cache that lives inside a helper where nobody
can see its TTL or flush it is worse than no cache at all, so it is its own named module.

What it holds and for how long:

* ``(organisation_id, credential_id) -> _ResolvedCredential`` for :data:`TTL_SECONDS`.
...
benefit: the embedder client is constructed fresh per call anyway, so the round-trips are the cost
worth removing, not the construction.
"""
...
if TYPE_CHECKING:  # annotations only — importing at runtime would be circular, since the resolver
    ...
    from oraclous_knowledge_retriever_service.services.embedder import _ResolvedCredential
...
_credentials: OrderedDict[tuple[str, str], tuple[float, _ResolvedCredential]] = OrderedDict()
...
def get_credential(organisation_id: uuid.UUID, credential_id: str) -> _ResolvedCredential | None:
...
def put_credential(organisation_id: uuid.UUID, credential: _ResolvedCredential) -> None:
    _put(
        _credentials,
