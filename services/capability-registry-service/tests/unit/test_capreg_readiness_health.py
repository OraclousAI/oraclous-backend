...
from oraclous_capability_registry_service.core import lifespan as lifespan_module
...
async def test_store_bind_failure_reports_degraded_and_alerts(monkeypatch, _capture_alerts):
    ...
    monkeypatch.setattr(lifespan_module, "CapabilityRepository", _boom)
    app = create_app()
...
async def test_healthy_startup_reports_healthy():
    app = create_app()
    app.state.capability_repository = object()
    health = await _get(app, "/health")
    assert health.status_code == 200
