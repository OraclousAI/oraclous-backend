"""Provision the NOSUPERUSER/NOBYPASSRLS runtime role + GRANTs (ADR-030 §3, core connection layer).
    ...
    python -m oraclous_capability_registry_service.core.bootstrap_rls_role
...
* ``GRANT USAGE ON SCHEMA public`` and ``GRANT SELECT, INSERT, UPDATE, DELETE`` on the four
  org-scoped capability-registry tables to ``oraclous_app`` (and on FUTURE tables via ALTER DEFAULT
  PRIVILEGES so a later migration's tables are covered without re-running this with new names).

The runtime capability-registry then connects as ``oraclous_app`` (a non-owner, NOSUPERUSER role),
so the FORCE'd RLS policy bites it. The OWNER keeps running migrations (it bypasses RLS as a
superuser), and the startup plugin-catalogue seed runs under the runtime role too — it inserts the
built-in tools into the PLATFORM_ORG with ``app.current_organisation_id`` bound to that org
(``org_scope``), so the strict WITH CHECK on ``capability_descriptors`` admits the seed (idempotent:
re-seeding is a no-op). This is the riskiest step in the rollout — a missing GRANT fails a service
closed at first query — so the grants are provisioned in lockstep with the RLS migration and the
runtime asserts its role at startup (``RLS_ASSERT_RUNTIME_ROLE``).
...
def main() -> None:
    ...
    with psycopg.connect(libpq_dsn, autocommit=True) as conn:
        ...
        # grant_all_tables=False: per-table DML grants on just the four RLS-enabled capreg tables.
        provision_app_role(
            conn,
            role=_APP_ROLE,
