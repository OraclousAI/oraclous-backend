...
async def test_store_bind_failure_reports_degraded_and_alerts(monkeypatch, _capture_alerts):
    ...
    monkeypatch.setattr(lifespan_module, "ExecutionRepository", _boom)
    app = create_app()
...
async def test_healthy_startup_reports_ok():
    # /health is liveness; the route only reads app.state.execution_repository.
    app = create_app()
    app.state.execution_repository = object()
    health = await _get(app, "/health")
    assert health.status_code == 200
