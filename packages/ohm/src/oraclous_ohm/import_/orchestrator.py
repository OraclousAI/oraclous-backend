...
subagents, so they are not emitted. Pure; fail-closed; flag-not-guess. Member sub-harness bodies and
merging with ``.claude/agents`` members + schedules are #408 — this emits the skeleton + DAG only.
"""
...
from oraclous_ohm.import_.mapping import slugify
from oraclous_ohm.import_.skills import ResolvedSkill, resolve_skill
from oraclous_ohm.manifest import OHMMember, OHMOrchestration, OHMTermination
...
def adapt_orchestrator_skill(
    ...
    for d in ordered:
        ...
        for f in wave_files:
            ...
            seen.add(role)
            subgoal = _first_heading(f.read_text(encoding="utf-8")) or role
            members.append(
                OHMMember(
                    role=role,
                    kind="agent",
            ...
            flag(
                "F-MEMBER-SUBHARNESS-DEFERRED",
                "info",
                f"member {role!r} skeleton; sub-harness body is #408",
                role,
            )
    ...
    return OrchestratorPlan(
        orchestration=orchestration, members=members, flags=flags, conditional_modes=modes
    )
