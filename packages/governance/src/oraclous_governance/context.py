...
auth-issued organisation claim on the principal (R1 agent tokens) or a
...
Shape reference: the org-context Contract (ratified Option B, 28 May 2026). Legacy
...
class Principal:
    ...
    tokens); ``None`` models the R0.5 identity-only principal whose
...
class MembershipResolver(Protocol):
    ...
    The concrete store lives in the substrate; it is injected
