...
#   scripts/e2e.sh --byom-smoke  # the real-model SUBSET a pull request runs (-m "byom and byom_smoke", #1012)
...
run_byom_smoke() {  # the SUBSET a pull request runs (#1012) — the same selection as the CI e2e job
  ...
  # citation path. The subset is the `byom_smoke` marker, so it is edited by moving a
  ...
  _load_search_key   # #881: a compiled team's researcher may need the live search key connected
  _recreate_harness live
  echo ">> BYOM real-LLM SUBSET through the gateway (live LLM, the PR-gate subset)…"
