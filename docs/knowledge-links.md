# Knowledge-base page index

The `oraclous-knowledge` git repository is canonical (see `CLAUDE.md` §2). The links below go to its **read-only Confluence mirror** for convenient browsing; when the mirror disagrees with `oraclous-knowledge` or with shipped reality, the knowledge repo wins.
...
| Per-service reference | `services/<service>/CLAUDE.md` (each links its own reference page) |
