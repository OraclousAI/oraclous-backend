...
**No e2e leg runs on a pull request (#1144).** The deployed-stack e2e runs once a night in `.github/workflows/e2e-nightly.yml` (deterministic + OAuth keyless, plus the full BYOM real-LLM leg). No PR waits on it and it is not a merge check. The local `scripts/e2e.sh` run (Rule 3) is the pre-PR proof; a red nightly is triaged the next morning.
