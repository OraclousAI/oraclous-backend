...
premise. Two things have to be true, and CI's unit suite can prove neither:
  ...
  DELIVERY (real BYOM) — the answers actually land in a real member's harness input, correctly
  split into given facts and unverified assumptions, through engine → worker → live harness. The
  member is asked only to COUNT each group, which is a reading task rather than a judgement, so
  the assertion holds on what arrived.
...
#: Two flagged, one given — deliberately ASYMMETRIC. An even split would still count 1 and 1 if the
#: two groups were swapped, so the counts would prove nothing about which heading each landed under.
_ANSWERS = [
    {"question": _Q_SEGMENT, "answer": None, "hypothesis": True},
    {"question": _Q_PRICE, "answer": "maybe $50/mo, unsure", "hypothesis": True},
...
def _counting_team(user: dict, credential_id: str) -> tuple[dict, dict]:
    """Import a real one-member agent asked only to COUNT the two groups in its briefing.

    A count, not an echo. The obvious test — "repeat the text you were handed, verbatim" — reads to
    a model like prompt extraction and gpt-4o-mini refuses it: three of four trial runs came back
    "I'm sorry, I can't assist with that", which is a flaky test, which is a bug. Counting is an
    ordinary reading task, and five of five trials returned the identical line.

    It also asserts MORE than an echo would. A render that let a confirmed answer drift under the
    unverified directive still echoes every word, but it counts 3 and 0 instead of 2 and 1 — and
    that mis-framing is the one outcome worse than sending nothing.
    """
    ...
    (agents / "researcher.md").write_text(
        "---\nname: researcher\n---\n"
        "Your briefing lists what the user told us about their situation. Some of those "
        "items are marked as unverified assumptions rather than given facts.\n"
        "Count each group. Reply on ONE line and nothing else, in this exact form:\n"
        "unverified=<count> given=<count>\n"
    )
...
def test_a_flagged_answer_reaches_a_real_member_marked_unverified(
    ...
    the silent discard #714 closed, and no unit test can see the difference — so this drives a LIVE
    harness and reads back what the member itself counted in the briefing it was handed.

    Two flagged, one given. Any other pair means the answers were dropped, merged into one block,
    or filed under the wrong heading.
    """
    user = register(f"answersbyom{uuid.uuid4().hex[:10]} user")
    c = gateway_client(user["token"])
    doc, subs = _counting_team(user, _credential(c, user))
    ...
    counted = str((done.get("results") or {}).get("researcher", {}).get("output", ""))
    assert "unverified=2" in counted, (
        f"the two flagged answers did not arrive under the unverified directive — {counted[:400]}"
    )
    assert "given=1" in counted, (
        f"the confirmed answer was dropped or mis-filed as an assumption — {counted[:400]}"
    )
