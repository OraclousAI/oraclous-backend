...
Tests currently failing (before ORAA-74 implementation):
  GDR-02  GoogleDriveReader OHM validation fails — type "oauth2" is not a valid
           CredentialType; the OHM spec requires "oauth_token"
  GDR-03  GoogleDriveReader credential type must be "oauth_token"
  GDR-04  GoogleDriveReader oauth_token must declare at least one scope (T2-M3)
  GDR-05  GoogleDriveReader scopes must include the drive.readonly URI
  PG-02   PostgreSQLReader OHM validation fails — type "database" is not a valid
           CredentialType; the OHM spec requires "connection_string"
  PG-03   PostgreSQLReader credential type must be "connection_string"
  MY-02   MySQLReader OHM validation fails — type "database" is not a valid
           CredentialType; the OHM spec requires "connection_string"
  MY-03   MySQLReader credential type must be "connection_string"
...
from ohm.schemas.credential import CredentialType
...
def test_google_drive_reader_descriptor_id_matches_plugin_id():
    ...
    assert descriptor.get("id") == GoogleDriveReader.get_plugin_id(), (
        ...
        f"Got descriptor id={descriptor.get('id')!r}, plugin_id={GoogleDriveReader.get_plugin_id()!r}"
    )
...
def test_notion_reader_descriptor_id_matches_plugin_id():
    ...
    assert descriptor.get("id") == NotionReader.get_plugin_id(), (
        f"descriptor['id'] must equal get_plugin_id(). "
        f"Got descriptor id={descriptor.get('id')!r}, plugin_id={NotionReader.get_plugin_id()!r}"
    )
