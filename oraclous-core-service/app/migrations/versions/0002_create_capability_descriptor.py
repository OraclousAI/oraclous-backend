...
def upgrade() -> None:
    ...
    op.execute(
        sa.text("CREATE INDEX ix_capability_descriptor_org_id ON capability_descriptor (org_id)")
    )
    op.execute(
        sa.text("CREATE INDEX ix_capability_descriptor_kind ON capability_descriptor (kind)")
    )
...
def downgrade() -> None:
    op.execute(sa.text("DROP INDEX IF EXISTS ix_capability_descriptor_kind"))
    op.execute(sa.text("DROP INDEX IF EXISTS ix_capability_descriptor_org_id"))
